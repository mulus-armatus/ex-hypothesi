...
\definecolor{ehAzul1}{rgb}{0.35,0.55,0.85}		% Azul oscuro
\definecolor{ehAzul2}{rgb}{0.75,0.85,0.95}		% Azul claro
...
\renewcommand{\thesection}{\arabic{section}}	% Modifica la numeración de las secciones para que sólo ponga el número de la sección y no el del capítulo (evita secciones 0.1, 0.2, 0.3, etc)
\titleformat
...
\newcommand{\printchapterauthor}[2]{%
	{\parindent0.8cm\vspace*{-45pt}%
		\linespread{1.1}\small\scshape#1\footnote{#2}%
		\par\nobreak\vspace*{50pt}}
	\@afterheading%
}
...
\makeatother
\newcommand{\infoCap}[4]{			% Argumentos: 1) Título del artículo 1 2) Título del artículo 2 
	...
	\chapter*{#1\\#2}				% Títuloombre del capítulo
	\addcontentsline{toc}{chapter}{\normalsize #1~#2}				% Añadir el título del capítulo al índice
	\chapterauthor{#3}{#4}			% Poner al autor del capítulo debajo del título y en el índice
	\fancyfoot[RE,RO]{{\footnotesize #1\\#2\\\scshape\vspace{-0.2cm}#3}}	% Poner título y autor al lado derecho del pie de página
	...
	\fancyhead[LE]{\hspace*{-0.2\headwidth}\colorbox{ehAzul2}{\makebox[\dimexpr0.2\headwidth-2\fboxsep][c]{\strut\textcolor{ehAzul1}{\bfseries\thepage}}}%
		\colorbox{ehAzul1}{\makebox[\dimexpr\headwidth-2\fboxsep][l]{\strut\scshape\textcolor{ehAzul2}{\bfseries#1\ldots}}}}
	\fancyhead[LO]{\colorbox{ehAzul1}{\makebox[\dimexpr\headwidth-2\fboxsep][r]{\strut\scshape\textcolor{ehAzul2}{\bfseries#1\ldots}}}%
...
% Librerías
\usepackage[sectionbib]{natbib}			% Librería para tener una bibliografía por capítulo
\usepackage{chapterbib}					% Hmmm, esta librería parece hacer lo que creo que hace la anterior, pero ambas son necesarias
\usepackage{apalike}					% Librería para usar APA en los artículos
...
\newcommand{\referencias}{
	...
	\bibliographystyle{apalike}											% Usar el estilo APA
	{\footnotesize \bibliography{tex/bibliografia-No1}}					% Especifica el archibo que contiene la información bibliográfica
}
...
\title{\textit{\textcolor{ehAzul1}{\textbf{ex}}\textcolor{ehAzul2}{$\rightarrow$}\textcolor{ehAzul1}{\vspace{-0.4cm}hypothesi}}}		 				% Título de la revista
\date{}								% No poner ninguna fecha para que no salga
...
% Los que más cambian
\newcommand{\numeroRevista}{1}										% El numero de la revista
\newcommand{\semestre}{2019-1}										% El semestre en el que se hizo el número
\newcommand{\representante}{Martín Buenahora Bonilla}				% Representante de estudiantes actual
\newcommand{\editores}{	Pablo Rivas Robledo\tabularnewline			% Miembros del comité editorial
						& Martín Buenahora Bonilla}
\newcommand{\profesores}{	Juan Camilo Espejo Serna\tabularnewline
...
\small					% La tipografía normal del documento es
						% small, no normalzise. Esto se debe a que
						% era la forma más sencilla de conseguir 
						% _este_ tamaño de letra. 10pt, 11pt, 12pt
						% etc. no daban este resultado, ya que siempre
						% era o muy grande o muy pequeño. Aunque esto
						% seguramente va a cambiar pronto, pero me
						% parece bueno recordar por qué carajos
						% hay un  small aquí.
